...
symlinkroot(struct inode *ip)
{
  ...
  for(int i=0; i<SYMLINKDEPTH; i++) {
    ...
    /** 没成环 */
    ilock(ip);
    if(ip->type != T_SYMLINK)  // 追溯到非符号链接节点，说明溯源成功 
      return ip;
  }  
...
sys_open(void)
{
  ...
  f->writable = (omode & O_WRONLY) || (omode & O_RDWR);
    // If opened with O_APPEND, start write offset at end of file.
...
sys_readlink(void)
{
  ...
  if(n < 0 || n > MAXPATH) {
    printf("readlink: invalid buffer size %d\n", n);
    return -1;
  }
  ...
  if((ip = namei(path)) == 0){
    printf("readlink: cannot find file %s\n", path);
    end_op();
    return -1;
  }
